...
在浏览器支持 ES 模块之前，开发者还没有一个可以以模块化方式编写 JavaScript 的原生机制。这也是我们有 “打包” 这个概念的原因：使用工具抓取、处理和链接我们的源代码模块到文件中，使其可以运行在浏览器中。

<!-- 第二段的分段真的有些生硬 -->

一段时间以来，我们见过像 [webpack](https://webpack.js.org/)、[Rollup](https://rollupjs.org) 以及 [Parcel](https://parceljs.org/) 等大大改进了前端开发者的开发体验的工具。

然而，当我们开始构建越来越宏大的应用时，我们所处理的 JavaScript 代码数量也成倍增加 —— 不少大型项目包含数千个模块。我们开始遇到基于 JavaScript 的工具的性能瓶颈：通常需要很长时间（有时长达几分钟！）才能启动开发服务器；即使是使用 HMR，文件编辑也需要几秒钟才能在浏览器中反映出来。缓慢的反馈会大大影响开发者的工作效率和幸福感。

Vite 旨在利用下面这些生态系统中的新进展解决上述问题：浏览器支持原生模块，越来越多 JavaScript 工具使用编译型语言编写。

### 缓慢的服务器启动

当冷启动开发服务器时，基于的方式是在提供服务前去急切地抓取和构建你的整个应用。
...
当基于打包器启动时，编辑文件后将重新构建文件本身。显然，我们不应该重新构建整个包 —— 这样应用体积一大更新速度就会直线下降。
...
一旦你体验到 Vite 的神速，你仍然愿意忍受像曾经那样使用打包器开发就成了我们十分怀疑的事了。
...
虽说 `esbuild` 快得惊人，并且已经是一个在构建库方面比较出色的工具，但一些针对构建 _应用_ 的重要功能仍然还在持续开发中 —— 特别是代码分割和 CSS 处理方面。就目前来说，Rollup 在应用打包方面更加成熟和灵活。尽管如此，当未来这些功能稳定后，我们也不排除使用 `esbuild` 作为生产构建器的可能。

## Vite 与 xxx 的区别是？

你可以查看 [比较](./comparisons) 章节，了解更多关于 Vite 与其他类似工具的不同之处。
