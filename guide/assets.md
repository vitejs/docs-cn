...
这种模式需要把esbuild构建目标设置为 `es2020` 或更高版本，`vite@2.x` 默认使用 `es2019` 作为构建目标，如果你打算使用这种模式请设置 [build-target](https://cn.vitejs.dev/config/#build-target) 和 [optimizedeps.esbuildoptions.target](https://cn.vitejs.dev/config/#optimizedeps-esbuildoptions)为 `es2020` 或更高版本
:::
