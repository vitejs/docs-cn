...
一小部分用户需要 [@vitejs/plugin-legacy](https://github.com/vitejs/vite/tree/v5/packages/plugin-legacy)，它会自动生成兼容性 chunk 以及相应的 ES 语言功能的 polyfill。
...
- 下列在 v2 当中我们已经标记为弃用选项，现在已经被移除：
  ...
  - `build.polyfillDynamicImport`（在没有支持动态导入的浏览器中，使用 [`@vitejs/plugin-legacy`](https://github.com/vitejs/vite/tree/v5/packages/plugin-legacy)）
...
Vite v3 默认在 SSR 构建时使用 ESM 格式。当使用 ESM 时，[SSR 外部化的启发式方法](https://v3.vite.dev/guide/ssr.html#ssr-externals)将不再需要。默认情况下所有的依赖都将被外部化。你可以使用 [`ssr.noExternal`](../config/ssr-options.md#ssrnoexternal) 来控制哪些依赖需要被包含进 SSR 的打包产物中。
...
请先查看 Vite v2 文档中的[从 v1 迁移指南](https://v2.vite.dev/guide/migration.html)（[本地中文版](/guide/migration-from-v1)），了解将应用迁移到 Vite v2 所需的更改，然后再处理本页提到的变化。
