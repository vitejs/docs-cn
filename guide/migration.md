...
如果你正在从 `rolldown-vite`（面向 Vite 6 和 Vite 7 的 Rolldown 集成技术预览版本）迁移，那么本页只有标题中包含 <Badge text="NRV" type="warning" /> 的部分适用。
...
这些浏览器版本符合 [Baseline 广泛可用](https://web-platform-dx.github.io/web-features/) 截至 2026-01-01 的功能集。换句话说，它们都发布于大约两年半前。
...
`rolldown-vite` 包提供了由 Rolldown 驱动的 Vite 7，但不包含 Vite 8 的其他变更。这可作为迁移到 Vite 8 的中间步骤。请参阅 Vite 7 文档中的 [Rolldown 集成指南](https://v7.vite.dev/guide/rolldown)，了解如何从 Vite 7 切换到 `rolldown-vite`。
...
目前，Oxc 转换器尚不支持对原生装饰器进行降级转换，因为我们仍在等待相关规范取得进展，详见 [oxc-project/oxc#9170](https://github.com/oxc-project/oxc/issues/9170)。

:::: details 原生装饰器降级转换的临时解决方案

目前可以使用 [Babel](https://babeljs.io/) 或 [SWC](https://swc.rs/) 暂时对原生装饰器进行降级转换。
...
`esbuild` 不再由 Vite 直接使用，现为可选依赖。如果你使用的插件调用了 `transformWithEsbuild` 函数，需要将 `esbuild` 安装为 `devDependency`。`transformWithEsbuild` 函数已被弃用，将来会被移除。我们建议改用新的 `transformWithOxc` 函数。
...
有关此问题的更多详细信息，请参阅 Rolldown 的文档：[CJS 模块中存在歧义的 `default` 导入 - 打包 CJS | Rolldown](https://rolldown.rs/in-depth/bundling-cjs#ambiguous-default-import-from-cjs-modules)。
...
### 移除使用格式探测的模块解析机制 {#removed-module-resolution-using-format-sniffing}
...
### UMD/IIFE 中的 `import.meta.url` {#import-meta-url-in-umd-iife}

在 UMD/IIFE 输出格式中，不再为 `import.meta.url` 提供 polyfill。默认情况下，它将被替换为 `undefined`。如果你更喜欢之前的行为，可以使用 [`define`](/config/shared-options#define) 选项配合 [`build.rolldownOptions.output.intro`](https://rolldown.rs/reference/OutputOptions.intro) 选项。有关更多详细信息，请参阅 Rolldown 的文档：[非 ESM 输出格式中的常见 `import.meta` 属性 | Rolldown](https://rolldown.rs/in-depth/non-esm-output-formats#well-known-import-meta-properties)。
...
### 移除 `build.rollupOptions.output.manualChunks` 的对象形式并弃用其函数形式 {#remove-object-form-build-rollupoptions-output-manualchunks-and-deprecate-function-form-one}

`output.manualChunks` 选项的对象形式不再支持。`output.manualChunks` 的函数形式已弃用。Rolldown 提供了更灵活的 [`codeSplitting`](https://rolldown.rs/reference/OutputOptions.codeSplitting) 选项。有关 `codeSplitting` 的更多详细信息，请参阅 Rolldown 的文档：[手动代码分割 - Rolldown](https://rolldown.rs/in-depth/manual-code-splitting)。

### `build()` 抛出 `BundleError` {#build-throws-bundleerror}

_此更改仅影响 JS API 用户。_

`build()` 现在抛出 [`BundleError`](https://rolldown.rs/reference/TypeAlias.BundleError)，而不是插件中抛出的原始错误。`BundleError` 的类型为 `Error & { errors?: RolldownError[] }`，它会将各个错误包装在 `errors` 数组中。若要访问这些错误，请使用 `.errors`：
...
- Rollup 中的所有并行钩子现在均按串行方式执行。详见 [Rolldown 的文档](https://rolldown.rs/apis/plugin-api#sequential-hook-execution)。
- `"use strict";` 有时不会被注入。详见 [Rolldown 的文档](https://rolldown.rs/in-depth/directives)。
- 不支持使用 `plugin-legacy` 转换到 ES5 及更低版本（[rolldown-vite#452](https://github.com/vitejs/rolldown-vite/issues/452)）。
...
请先查阅 Vite v7 文档中的 [从 v6 迁移指南](https://v7.vite.dev/guide/migration)（[中文版](/guide/migration-from-v6.md)），了解将你的应用迁移到 Vite 7 所需的变更，然后再继续执行本页中的相关更改。
