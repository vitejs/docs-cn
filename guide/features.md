...
1. [预构建](./dep-pre-bundling) 它们可以提高页面加载速度，并将 CommonJS / UMD 转换为 ESM 格式。预构建这一步由 [Rolldown](https://rolldown.rs/) 执行，这使得 Vite 的冷启动时间比任何基于 JavaScript 的打包器都要快得多。
...
Vite 使用 [Oxc 转换器](https://oxc.rs/docs/guide/usage/transformer.html) 将 TypeScript 转译到 JavaScript，其速度比原生 `tsc` 更快，同时 HMR 更新反映到浏览器的时间小于 50ms。
...
这是因为 Oxc 转换器只执行没有类型信息的转译，它并不支持某些特性，如 `const enum` 和隐式类型导入。
...
要在开发中指定目标，可使用 [`oxc.target`](/config/shared-options.html#oxc) 选项，默认值为 `esnext`，以实现最小的转译。在构建中，[`build.target`](/config/build-options.html#build-target) 选项优先于 `oxc.target`，如有需要也可以进行设置。
...
>>>>>>> 332667ac2a7cde8eeb866140f9a0191bd7dcc883
...
更多细节详见 [Oxc 转换器文档](https://oxc.rs/docs/guide/usage/transformer/jsx.html)。
...
同样也有对完全使用 Lightning CSS 进行 CSS 处理的实验性支持。你可以通过添加 [`css.transformer: 'lightningcss'`](../config/shared-options.md#css-transformer) 来选择启用它。

如需配置，可以将 Lightning CSS 的选项传递给 [`css.lightningcss`](../config/shared-options.md#css-lightningcss) 配置项。要配置 CSS Modules，需要使用 [`css.lightningcss.cssModules`](https://lightningcss.dev/css-modules.html) 而不是 [`css.modules`](../config/shared-options.md#css-modules)（后者是用于配置 PostCSS 处理 CSS Modules 的方式）。
