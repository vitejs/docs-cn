...
function translateLegacyThemeLabels() {
  ...
  document
    .querySelectorAll<HTMLElement>('.VPSwitchAppearance[title="toggle dark mode"]')
    .forEach((element) => element.setAttribute('title', '切换深色模式'))
...
onMounted(() => {
  ...
  observer = new MutationObserver(translateLegacyThemeLabels)
...
<template>
  <div class="old-document" role="status">
    <p>
      当前页面是 Vite 4 的<strong>历史版本文档</strong>。请前往
