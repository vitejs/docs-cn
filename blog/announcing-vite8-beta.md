...
摘要：由 [Rolldown](https://rolldown.rs/) 驱动的 Vite 8 首个 beta 版本现已发布。Vite 8 显著提升了生产环境构建速度，并为未来的改进奠定了基础。你可以将 `vite` 升级到 `8.0.0-beta.0` 版本并阅读 [迁移指南](https://main.vite.dev/guide/migration) 来体验新版本。

---

我们很高兴发布 Vite 8 的首个 beta 版本。此版本统一了底层工具链，带来了更一致的行为，并显著提升了构建性能。Vite 现在使用 [Rolldown](https://rolldown.rs/) 作为其打包工具，取代了之前 esbuild 和 Rollup 的组合。
...
由于 Vite 8 涉及核心构建行为，我们着重保持配置 API 和插件钩子不变。我们创建了 [迁移指南](https://main.vite.dev/guide/migration) 来帮助你进行升级。
...
> [!IMPORTANT]
> 如果你依赖特定的 Rollup 或 esbuild 选项，你可能需要对 Vite 配置进行一些调整。请参考 [迁移指南](https://main.vite.dev/guide/migration) 获取详细的说明和示例。
...
- **内置 tsconfig `paths` 支持**：开发者可以通过将 [`resolve.tsconfigPaths`](https://main.vite.dev/config/shared-options#resolve-tsconfigpaths) 设置为 `true` 来启用此功能。此功能会略微影响性能，因此默认情况下未启用。
- **`emitDecoratorMetadata` 支持**：Vite 8 现在内置了对 TypeScript [`emitDecoratorMetadata` 选项](https://www.typescriptlang.org/tsconfig/#emitDecoratorMetadata) 的自动支持。更多详情请参见 [功能](https://main.vite.dev/guide/features#emitdecoratormetadata) 页面。

## 展望未来 {#looking-ahead}

速度一直是 Vite 的一大特色。与 Rolldown 以及 Oxc 的集成意味着 JavaScript 开发者可以充分利用 Rust 的速度优势。升级到 Vite 8 后，仅凭底层改用 Rust 就能获得性能提升。

我们也即将推出 Vite 的完整打包模式，该模式将大幅提升 Vite 开发服务器在大型项目中的速度。初步测试结果显示，开发服务器启动速度提升 3 倍，完全重新加载速度提升 40%，网络请求次数降至原来的十分之一。

Vite 的另一大特色是其插件生态系统。我们希望 JavaScript 开发者能够继续使用他们熟悉的 JavaScript 语言来扩展和定制 Vite，同时又能受益于 Rust 的性能优势。我们的团队正在与 VoidZero 团队合作，以提升 JavaScript 插件在这些基于 Rust 的系统中的运行效率。
