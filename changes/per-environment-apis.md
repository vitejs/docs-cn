# 迁移到基于环境的 API {#move-to-per-environment-apis}
...
::: warning 即将废弃
`环境`实例首次在 `v6.0` 中引入。计划在未来的主要版本中废弃现在的 `server.moduleGraph` 和其他方法。我们不建议你现在就放弃 server 方法。要识别你的使用情况，请在你的 Vite 配置中设置以下。
...
在 Vite v6 中，现在可以创建任意数量的自定义环境（`client`、`ssr`、`edge` 等）。单一的 `ssr` 布尔值已经不够用了。我们没有将 API 改为 `server.transformRequest(url, { environment })`，而是将这些方法移到了环境实例中，这样就可以在没有 Vite 开发服务器的情况下调用这些方法。
